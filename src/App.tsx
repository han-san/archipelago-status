...
async function login(url: string, name: string, connectedClients: Map<string, PlayerInfo>): Promise<Map<string, PlayerInfo>> {
  const client = new Client()

  await client.login(url, name).then(async () => {
    const player = client.players.self
    console.log(`Logged in as ${player.name}(${player.alias}) playing ${player.game}`)
    ...
    if (connectedClients.size < 2) {
      ...
      const promises = []
      for (const slotInfo of Object.values(client.players.slots).filter(si => si.name !== name)) {
...
function App() {
  const [connectedClients, setConnectedClients] = useState(new Map<string, PlayerInfo>);
  ...
  function playerHintNode(hint: Hint) {
    ...
    return <li key={hint.item.toString()} className={classes}>
  ...
  function playerInfoNode(playerInfo: PlayerInfo) {
    ...
    */
    ...
    return <div key={playerInfo.client.players.self.slot}>
      <div>
        <span className="alias">{playerInfo.client.players.self.alias}</span>
      </div>
      <div className="game">
        {playerInfo.client.game}
      </div>
      <div>Hints:</div>
      <ul className="hints">
        {
  ...
  return (
    <>
      ...
      <div>
        <label htmlFor="login-url-input">Enter Archipelago server url and port</label>
        <input type="text" id="login-url-input" placeholder="192.168.1.107:38281"></input>
      </div>
      <div>
        <label htmlFor="login-player-name-input">Enter player name</label>
        <input type="text" id="login-player-name-input" placeholder="Login as player"></input>
      </div>
      <div>
        <button onClick={
          async () => {
            const loginPlayerNameInput = document.getElementById("login-player-name-input") as HTMLInputElement
            const loginUrlInput = document.getElementById("login-url-input") as HTMLInputElement
            ...
            for (const [playerName, info] of newMap) {
              info.client.items.on("hintReceived", (hint) => setConnectedClients((oldConnectedClients) => {
